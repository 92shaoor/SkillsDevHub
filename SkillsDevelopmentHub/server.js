...
app.post('/signup', async (req, res) => {
    const { firstname, lastname, email, address, phone, profession, password, confirmPassword } = req.body;
...
app.post('/book', async (req, res) => {
    if (!req.session.user || !req.session.user.email) {
        return res.status(401).send('You must be logged in to book a consultation.');
    }
...
app.post('/forgot-password', async (req, res) => {
    ...
    // Find user by email
    const user = await User.findOne({ email });
    if (!user) return res.json({ message: 'A reset link has been sent on the email.' });
    ...
    // Send email (configure transporter with your SMTP)
    ...
    res.json({ message: 'A reset link has been sent on the email.' });
});
...
app.put('/my-profile', async (req, res) => {
    ...
    const { firstname, lastname, address, phone, profession } = req.body;
    try {
        await User.updateOne(
            { email: req.session.user.email },
            { $set: { firstname, lastname, address, phone, profession } }
        );
...
app.get('/api/search', async (req, res) => {
    ...
    // Skills
    const skills = await Skill.find({ skillName: { $regex: query, $options: 'i' } });
    // Experts
    const experts = await Expert.find({ expertName: { $regex: query, $options: 'i' } });
    // Courses
    ...
    const results = [
        ...skills.map(skill => ({
            ...
            url: `SkillsList.html?skill=${encodeURIComponent(skill.skillName)}`
        })),
        ...experts.map(expert => ({
            type: 'Expert',
            ...
            url: `experts.html?expert=${encodeURIComponent(expert.expertName)}`
        })),
        ...courses.map(course => ({
            type: 'Course',
            name: course.courseName,
            details: course.details,
            url: `courses.html?course=${encodeURIComponent(course.courseName)}`
        }))
    ];
